...
) = TagCompoundBuilder(name, entries, lenient).apply(builder).build()

@NbtAPI
open class TagCompoundBuilder(private val name: String?, entries: CompoundMap, @PublishedApi internal val lenient: Boolean) {
	...
	inline fun compound(name: String, entries: CompoundMap = emptyMap(), builder: TagCompoundBuilder.() -> Unit) {
		this.entries[name] = TagCompoundBuilder(name, entries, lenient).apply(builder).build()
	}
	
	@NbtAPI
	inline fun compound(entries: CompoundMap = emptyMap(), builder: TagCompoundBuilder.() -> Unit) =
		TagCompoundBuilder(null, entries, lenient).apply(builder).build()
		...
		inline operator fun <reified T> set(name: String, value: List<T>) {
			set(name, when (T::class.java) {
				...
				TagLongArray::class.java      -> TagList(TAG_LONG_ARRAY, value as List<TagLongArray>, false, null)
					...
					if (lenient) {
						val lenientList = runTry { value.map { TagDouble((it as Number).toDouble(), null) } }
						                  ?: throw IllegalArgumentException("Unsupported type ${T::class.java.simpleName} for TagList")
						TagList(TAG_DOUBLE, lenientList, false, null)
